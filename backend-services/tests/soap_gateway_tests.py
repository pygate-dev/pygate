...
class TestDoorman:
    ...
    async def test_onboard_endpoint(self):
        TestDoorman.endpoint_path = "/NumberConversion.wso"
        response = requests.post(f"{self.base_url}/platform/endpoint", 
                                ...
                                cookies=TestDoorman.getAccessCookies(),
                                 json={
                                    ...
                                    "endpoint_description": "Test SOAP endpoint",
                                    "endpoint_parameters": {
                                        "body": [
                                            {"name": "ubiNum", "required": True, "type": "string"}
                                        ]
                                    }
                                 }, verify=False)
        assert response.status_code == 201
    ...
    async def test_create_routing(self):
        ...
        response = requests.post(f"{self.base_url}/platform/routing",
                                ...
                                json={
                                    ...
                                    "routing_servers": ["https://www.dataaccess.com/webservicesserver"],
                                }, verify=False)
        assert response.status_code == 201
    ...
    async def test_gateway_call_client_key(self):
        response = requests.post(f"{self.base_url}/api/soap/" + TestDoorman.api_name + "/v1" + TestDoorman.endpoint_path,
                                headers={
